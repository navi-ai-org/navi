...
| 2 | `unsafe` blocks lack `// SAFETY:` comments | `navi-plugin-host/src/lib.rs`, `navi-plugin-api/src/lib.rs` | PENDING |
...
| 6 | `navi-cli/acp.rs` imports from navi-core when SDK re-exports exist | `navi-cli/src/acp.rs` | PENDING |
...
| 18 | `OpenAiProvider` missing `Debug` derive | `navi-openai/src/provider.rs` | PENDING |
