...
| 2026-06-28 | 4 | Completed | Native plugin `register_agent_policy` declarations are consumed by the SDK for known runtime policies (`learning_tutor`, `navi_learning`, `tutor`, `default`, `code_agent`). TUI component declarations are preserved by the SDK per session and exposed for `navi-tui` to render. |
...
| 2026-06-28 | 5 | Completed | Added `navi-napi` crate, TypeScript engine builder, structured learning runtime options, lifecycle hooks, session/control methods, async TypeScript host-tool callbacks, and event stream `next()` support. |
