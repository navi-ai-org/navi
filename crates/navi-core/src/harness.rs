...
fn build_system_prompt_inner(
    ...
    let mut prompt = format!(
        concat!(
            ...
            "\n",
            "Workflow contract:\n",
            "1. Understand the task, then check the Available Skills catalog for a skill whose name, description, or tags match the user request. If a relevant skill exists, call `load_skill` (or `skill_list` then `load_skill`) to read its full instructions and follow them before acting.\n",
            ...
            "3. Structure/symbols: use `code` / `code_edit` / `code_exec` directly, or call tool_search for `ast_search` / `symbol_*` schemas.\n",
