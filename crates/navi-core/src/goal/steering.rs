...
/// Builds a continuation steering prompt when the thread is idle with an active goal.
...
Progress visibility:
If `plan` is available and the next work is meaningfully multi-step, use it to show a concise plan tied to the real objective. Keep the plan current as steps complete or the next best action changes. Skip planning overhead for trivial one-step progress, and do not treat a plan update as a substitute for doing the work.
