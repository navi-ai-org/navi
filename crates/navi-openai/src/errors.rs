    ...
    let user_message = match (status_code, provider_code) {
        ...
        (426, _) => {
            if body.contains("outdated") || body.contains("Grok CLI version") {
                "xAI rejected the request as an outdated Grok CLI client (HTTP 426). Update NAVI (x-grok-client-version), re-run xAI OAuth for Grok Build, or use an XAI_API_KEY from console.x.ai for Platform API.".to_string()
            } else {
