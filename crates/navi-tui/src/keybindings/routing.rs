...
pub(crate) fn route_key(app: &mut TuiApp, code: KeyCode, modifiers: KeyModifiers) -> KeyOutcome {
    ...
    // Route key through copland PanelManager. This gives copland overlays a
    // chance to handle keys before the existing mode-based key routing. If a
    // panel handles the key, we stop.
    //
